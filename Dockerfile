...
RUN echo 'tan' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/tan.sh

RUN echo 'cd ' >>/tan.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/tan.sh

RUN echo 'cd /noVNC-1.2.0' >>/tan.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/tan.sh

RUN chmod 755 /tan.sh
